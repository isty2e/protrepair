...
class MicrostateHydrogenPlacement:
    ...
    geometry : AtomGeometry
        ...
        original geometry, unless the patch explicitly requests repositioning.
...
class PolymerMicrostatePatch:
    ...
    reposition_hydrogens : frozenset[str]
        Final H names whose coordinates are explicitly regenerated by a placement
        action. Empty for chemistry-only edits. Identity, isotope, atom scalars
        and original observations remain preserved even when coordinates change.
    ...
    site: PolymerMicrostateSite
    hydrogens: tuple[MicrostateHydrogenPlacement, ...]
    reposition_hydrogens: frozenset[str] = frozenset()
    ...
    def __post_init__(
        ...
        object.__setattr__(
            self, "reposition_hydrogens", frozenset(self.reposition_hydrogens)
        )
        if not self.reposition_hydrogens <= names:
            raise ValueError("microstate repositioning must target final site H atoms")
    ...
    def _materialize(
        ...
        for entry in self.hydrogens:
            ...
            if name in self.reposition_hydrogens:
                placed = placed.with_position(entry.geometry.position)
