"""Local-refinement proposal policy for workflow planning."""
...
class LocalRefinementProposalPolicy:
    """Default search policy for local-refinement proposal candidates."""

    severe_geometry_fallback_limit: int = 1
    joint_scope_fallback_limit: int = 1
    severe_geometry_context_radius_angstrom: float = 3.0

    def __post_init__(self) -> None:
        _validate_candidate_limit(
            self.severe_geometry_fallback_limit,
            field_name="severe_geometry_fallback_limit",
        )
        _validate_candidate_limit(
            self.joint_scope_fallback_limit,
            field_name="joint_scope_fallback_limit",
        )
        object.__setattr__(
            self,
            "severe_geometry_context_radius_angstrom",
            _normalize_positive_radius(
                self.severe_geometry_context_radius_angstrom,
                field_name="severe_geometry_context_radius_angstrom",
            ),
        )
    ...
    def automatic_specs(
        ...
        """Return automatic repair-refinement specs for parser-visible burdens."""

        if not self.automatic_is_admissible(domain):
            return ()
        ...
        for proposal in batched_joint_scope_proposals[
            : self.joint_scope_fallback_limit
        ]:
            scope_spec = proposal.as_local_scope_spec()
            execution_scope_spec = execution_scope_spec_for_joint_correction(
...
def _validate_candidate_limit(value: int, *, field_name: str) -> None:
    """Validate one non-negative integer candidate cap."""
...
def _normalize_positive_radius(value: float, *, field_name: str) -> float:
    """Return one positive finite radius value."""
